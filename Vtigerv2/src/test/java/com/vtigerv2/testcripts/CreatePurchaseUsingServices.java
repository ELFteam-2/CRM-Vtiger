...
public class CreatePurchaseUsingServices extends BaseClass {
	@Test
	public void testCreatePurchaseUsingServices() throws InterruptedException, EncryptedDocumentException, InvalidFormatException, IOException {
	    ...
	    Reporter.log("Under Item Details click on Add Service Button",true);
	    np.getAddservicesbtn().click();
	    np.getServicesicon().click();
	    ...
	    Reporter.log("Click on Move Upward Arrow button (Delete the Product Item)",true);
	    np.getArrowicon().click();
	    np.getDeleteicon().click(); 
	    Reporter.log("Click on Save.",true);
	    np.getSavebtn().click();
	    Reporter.log("Purchase order is created using service",true);
	}
}
